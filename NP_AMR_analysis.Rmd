...
make_cluster_freq_wide_species <- function(mcat_strains_freq,
                                           species_col  = "Species",
                                           clusters_col = "Cluster",
                                           cohort_col   = "Cohort") {
  # Aggregate to one frequency per (Species, Cohort, Cluster)
  mcat_cluster_freq <- mcat_strains_freq %>%
    group_by(.data[[species_col]],.data[[cohort_col]],.data[[clusters_col]]) %>%
    summarise(
      Strains_count = sum(Strains_count, na.rm = TRUE),.groups = "drop"
    ) %>%
    group_by(.data[[species_col]],.data[[cohort_col]]) %>%
    mutate(
      Frequency = Strains_count / sum(Strains_count)
    ) %>%
    ungroup()
...
make_pair_panel_species <- function(wide_df,
  ...
  if (nrow(df_pair) >= 2) {
    fit   <- lm(df_pair[[cohort_y]] ~ df_pair[[cohort_x]])
...
make_diag_rank_panel <- function(wide_df,
                                 ...
                                 species1,
                                 species2,
                                 species_palette = c(species1 = "steelblue", species2 = "darkorange")) {
  # Gather for both species
  df_long <- wide_df %>%
    filter(Species %in% c(species1, species2)) %>%
    select(Species, Cluster, all_of(cohort)) %>%
    rename(Frequency = all_of(cohort)) %>%
    mutate(Frequency = replace_na(Frequency, 0))

  # Compute rank per species (descending frequency)
  df_ranked <- df_long %>%
    group_by(Species) %>%
...
plot_mcat_cluster_pairs_two_species <- function(mcat_strains_freq,
                                                ...
                                                species_palette = c(species1 = "steelblue",
                                                                    species2 = "darkorange"),
                                                row_exclude = "Drakenstein") {
  # 1. Build wide table with Species + Cluster + cohorts
  ...
  # Rows: all cohorts except row_exclude (e.g. "Drakenstein")
  row_cohorts <- setdiff(all_cohorts, row_exclude)

  n_rows <- length(row_cohorts)
  ...
  for (row_idx in seq_len(n_rows)) {
    for (col_idx in seq_len(n_cols)) {
      cx <- col_cohorts[col_idx]  # column cohort
      cy <- row_cohorts[row_idx]  # row cohort

      if (cx == cy) {
        # Diagonal: rank-frequency plots for both species in this cohort
        p <- make_diag_rank_panel(
          wide_df,
          cohort          = cx,
          species1        = species1,
          species2        = species2,
      ...
      # X-axis cohort labels on bottom row only
      if (row_idx == n_rows) {
        p <- p +
          labs(x = cx) +
          theme(
  ...
  patchwork::wrap_plots(panels, nrow = n_rows, ncol = n_cols)
}
...
p <- plot_mcat_cluster_pairs_two_species(
  ...
  species_palette = c("Moraxella catarrhalis" = "steelblue",
                      "Haemophilus influenzae"  = "darkorange"),
  row_exclude = "Drakenstein"
)
...
included_hflu <- unique(Hflu_version4_gene_preabsence$npg_run_id)
...
included_mcat <- unique(Mcat_version4_gene_preabsence$npg_run_id)
...
- Seroresistant and serosensitive co-colonisation?
