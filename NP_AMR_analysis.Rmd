...
treatment_df <-
  read.csv("/Users/ncrouche/Documents/AMR_MGE/pneumo_withinhost_manuscript/data/epi/Pneumo-DEEP_illness_abx_summary.csv")

maela_samples_df <-
  read.csv("/Users/ncrouche/Documents/AMR_MGE/pneumo_withinhost_manuscript/data/epi/Pneumo-DEEP_swab_summary.csv")

treatment_sampling_df <-
  maela_samples_df %>%
    dplyr::select(specnum,codenum,specdate) %>% 
    dplyr::mutate(isolate_date = lubridate::as_date(specdate, format = "%m/%d/%y")) %>% 
  dplyr::full_join(
    treatment_df %>% 
      dplyr::select(codenum,date_start) %>% 
      dplyr::filter(!is.na(date_start)) %>% 
      dplyr::mutate(
        date_start = lubridate::as_date(date_start, format = "%m/%d/%y")
      ),
    by = c("codenum"),
  ...
  dplyr::rename(treatment_date = date_start) %>% 
  dplyr::mutate(
    interval_before_treatment = treatment_date - isolate_date,
    interval_after_treatment = isolate_date - treatment_date
  ) %>% 
  dplyr::mutate(
    days_before_treatment = dplyr::if_else(as.numeric(interval_before_treatment) >= 0,
                                           as.numeric(interval_before_treatment),
                                           NA_real_),
    days_after_treatment = dplyr::if_else(as.numeric(interval_after_treatment) >= 0,
                                          as.numeric(interval_after_treatment),
                                          NA_real_)
  ) %>% 
  dplyr::group_by(specnum) %>%
  dplyr::mutate(
    days_before_treatment = min(days_before_treatment,na.rm = TRUE),
    days_after_treatment = min(days_after_treatment,na.rm = TRUE)
  ) %>% 
  dplyr::filter(as.numeric(interval_before_treatment) == days_before_treatment) %>% 
  dplyr::ungroup() %>% 
  dplyr::select(-c(interval_before_treatment,interval_after_treatment)) %>% 
  dplyr::distinct()
