...
maela_samples_df <-
  read.csv("./sample_data/Pneumo-DEEP_swab_summary.csv")
...
treatment_sampling_df <-
  ...
  dplyr::full_join(
    treatment_df %>% 
      dplyr::select(date_start,nps_specnum,drug1,drug2) %>% 
      dplyr::filter(!is.na(date_start) & !is.na(nps_specnum)) %>% 
      dplyr::mutate(
        date_start = lubridate::as_date(date_start, format = "%m/%d/%y")
      ) %>% 
      dplyr::rename(specnum = nps_specnum),
    by = c("specnum"),
