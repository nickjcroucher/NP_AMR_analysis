...
# Access data
icd /seq/ont/promethion/offline-deplexes/PC24B148/ONTRUN-196/TRAC-2-7738/20240219_1036_3A_PAU55810_bf5bbe22/dorado/7.1.4/sup/simplex/modified/5hmc_5mc_cg/default/

# Move data to scratch
cd /data/pam/team284/nc3/scratch/NP_AMR/data/nanopore_data/run_1

for DIR in $(ils); do
  if [[ $DIR =~ chunk ]]; then
    icd $DIR;
    IPWD_NAME=`ipwd`;
    CHUNK_NAME=`echo $IPWD_NAME | perl -lane 'my @d = split(/\//,$_); print $d[$#d];'`;
    for BARCODE in $(ils); do
      if [[ $BARCODE =~ barcode ]]; then
        icd $BARCODE;
        IPWD_NAME=`ipwd`;
        BARCODE_NAME=`echo $IPWD_NAME | perl -lane 'my @d = split(/\//,$_); print $d[$#d];'`;
        cd $BARCODE_NAME;
        for FILE in $(ils); do
          iget -N 1 $FILE;
          mv $FILE ${CHUNK_NAME}_${FILE};
        done;
        cd ..;
        icd ..;
      fi;
