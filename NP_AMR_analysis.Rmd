...
The long-read data were assembled with `flye` version 2.9.2, subsampling reads to get ~120-fold coverage of the genomes:

``` {bash Assembling long-read data, eval = FALSE}
rm run_1_flye.r120.*.[oe]; declare -i N; N=1; for f in $(ls -d barcode*); do echo -en "cd /data/pam/team284/nc3/scratch/NP_AMR/data/nanopore_data/run_1/$f\nmodule load flye/2.9.2--py310h590eda1_0\nFILES=\$(find -maxdepth 1 -type f -name 'chunk*fastq.gz' -printf '%s\t%f\n' |  cut -f2- | tr '\n' ' ')\ncat \$FILES > all.fastq.gz\nflye -t 4 --genome-size 2.0m --asm-coverage 120 --nano-raw all.fastq.gz --out-dir ${f}_r120_out\nrm all.fastq.gz\n" > flye_job.$N; chmod +x ./flye_job.$N; N=($N+1); done
bsub -n 4 -M 32000 -R 'select[mem>32000] rusage[mem=32000] span[hosts=1]' -o run_1_flye.r120.%I.o -e run_1_flye.r120.%I.e -J flye_job[1-96] ./flye_job.\${LSB_JOBINDEX}; watch bjobs

rm run_2_flye.r120.*.[oe]; declare -i N; N=1; for f in $(ls -d barcode*); do echo -en "cd /data/pam/team284/nc3/scratch/NP_AMR/data/nanopore_data/run_2/$f\nmodule load flye/2.9.2--py310h590eda1_0\nFILES=\$(find -maxdepth 1 -type f -name 'chunk*fastq.gz' -printf '%s\t%f\n' |  cut -f2- | tr '\n' ' ')\ncat \$FILES > all.fastq.gz\nflye -t 4 --genome-size 2.0m --asm-coverage 120 --nano-raw all.fastq.gz --out-dir ${f}_r120_out\nrm all.fastq.gz\n" > flye_job.$N; chmod +x ./flye_job.$N; N=($N+1); done
bsub -n 4 -M 32000 -R 'select[mem>32000] rusage[mem=32000] span[hosts=1]' -o run_2_flye.r120.%I.o -e run_2_flye.r120.%I.e -J flye_job_2[1-96] ./flye_job.\${LSB_JOBINDEX}; watch bjobs

```

The quality of the assemblies was assessed using Quast v5.3:

``` {bash Evaluate assemblies, eval = FALSE}

module load quast/5.3.0
quast.py -o long_read_assemblies_v1 *.fa

```

These genomes were assembled across two Promethion runs - the best assembly was selected across the two runs:

``` {r Long read assembly QC}

lr_assembly_stats_df <-
  read.table(file = "./sample_data/NP_AMR_long_read_assembly_stats.tsv",
             comment.char = "",
             check.names = FALSE,
             sep = "\t",
             header = TRUE) %>% 
  tidyr::pivot_longer(cols = -Assembly,names_to = "Dataset", values_to = "Value") %>% 
  dplyr::filter(Assembly %in% c("Largest contig","# contigs")) %>% 
  dplyr::mutate(
    Run = dplyr::if_else(grepl("run_1",Dataset),1,2),
    Barcode = str_split_i(Dataset, "_", i = 3)
  ) %>% 
  dplyr::mutate(
    BarcodeNumeric = as.numeric(
      as.character(
        gsub("barcode","",Barcode)
      )
    )
  ) %>% 
  dplyr::mutate(Assembly =
                  dplyr::if_else(
                    Assembly == "# contigs",
                    "n_contigs",
                    "Largest contig"
  ...
  tidyr::pivot_wider(names_from = Assembly,
                     values_from = Value) %>%
  dplyr::group_by(Barcode) %>% 
  dplyr::slice_max(`Largest contig`) %>% 
  dplyr::ungroup()

```

We can then associate the long-read assemblies with the sample information:

``` {r Combine long read assemblies with sample data}
...
We can load the code needed for methylation analysis: `microbemod/1.0.3`.
