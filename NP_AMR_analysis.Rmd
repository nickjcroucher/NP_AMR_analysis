...
First, the distances were calculated for all the *M. catarrahlis* genomes using PopPUNK version 2.7.7:
...
``` {r Clustering of plasmid sequences with}

plasmid_clustering_df <-
  read.csv("/Users/ncrouche/Downloads/NP_AMR_plasmid_clustering_results.csv") %>% 
  dplyr::mutate(
...
ggplot(plasmid_clustering_df,
  ...
  geom_point() +
  geom_mark_ellipse(
    expand = 0.02,     # how much to expand around the points
...
plasmid_clustering_by_isolate_df <-
  plasmid_clustering_df %>% 
    dplyr::select(npg_run_id,`Plasmid type`) %>% 
    dplyr::filter(!is.na(`Plasmid type`)) %>% 
    dplyr::mutate(Presence = "Present") %>% 
    tidyr::pivot_wider(id_cols = npg_run_id,
                       names_from = `Plasmid type`,
...
Mcat_version4_gene_preabsence <- read.csv(
  #tmp_path,
  "~/Downloads/gene_presence_absence_Mcat.csv",
  header = TRUE,
  comment.char = "",
  check.names = FALSE,
  sep = ","
