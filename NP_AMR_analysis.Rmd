...
long_read_info_df <-
  read.csv("./sample_data/long_read_sequence_data.csv") %>% 
    ...
    dplyr::mutate(Run = dplyr::if_else(grepl("run_1",Run),1,2)) %>% 
...
We then aggregate all the assemblies in one place:

``` {bash, eval = FALSE}

for DIR in run_1 run_2; do cd $DIR; for SUBDIR in $(ls -d barcode*); do FILE=`ls ./$SUBDIR/*120*/assembly.fasta`; if [[ -e $FILE ]]; then cp $FILE /data/pam/team284/nc3/scratch/
NP_AMR/data/nanopore_data/assemblies/${DIR}_${SUBDIR}.fasta; fi; done; cd ..; done
...
quast.py -o long_read_assemblies_v1 *.fasta
