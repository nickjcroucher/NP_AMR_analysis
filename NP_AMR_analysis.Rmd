...
merge_clusters_by_distance <- function(df, dist_df, plasmid_dist_threshold, size_diff_threshold) {
  ...
  components_df <- tibble(
    ...
    merged_cluster = igraph::components(g)$membership
  )
  
  # --- Join back to original df ---
  df |>
    dplyr::left_join(components_df, by = "Sample_Name")
}
...
sample_assemblies_df <- 
  dplyr::right_join(as_tibble(assembly_stats_df),
             as_tibble(sample_df),
             by = "npg_run_id")
...
``` {r Decide on clustering of plasmids}
...
cluster_sizes <- plasmid_reclustering_df |>
  dplyr::count(merged_cluster)

small_clusters <- cluster_sizes |> dplyr::filter(n <= 25) |> dplyr::pull(merged_cluster)
large_clusters <- cluster_sizes |> dplyr::filter(n >  25) |> dplyr::pull(merged_cluster)
...
ggplot(
  ...
  ggforce::geom_mark_ellipse(
    data      = \(x) dplyr::filter(x, merged_cluster %in% small_clusters &
  ...
  ggforce::geom_mark_hull(
    data      = \(x) dplyr::filter(x, merged_cluster %in% large_clusters &
                                      !is.na(merged_cluster)),
    aes(group = new_plasmid_name),
    expand    = 0.015,
    alpha     = 0.15,
  ...
  ggrepel::geom_label_repel(
    ...
    nudge_x            = cluster_centroids$nudge_x*0.5 * sign(cluster_centroids$tsne1D),
    nudge_y            = cluster_centroids$nudge_y*0.5 * sign(cluster_centroids$tsne2D),
    max.overlaps       = Inf,
    min.segment.length = 0,
    segment.size       = 0.3,
    segment.colour     = NA     # <-- hide segments if drawn
  ) +
  scale_colour_manual(values = species_palette)
...
long_read_sample_df <-
  read.csv("./sample_data/long_read_sequence_data.csv") %>% 
    dplyr::filter(Project %in% c("References","Moraxella passage")) %>% 
    dplyr::mutate(`Parental genotype` = word(Description, 3)) %>% 
    dplyr::left_join(sample_df %>% 
                       dplyr::filter(Description == "Single isolate") %>% 
                       dplyr::mutate(`Parental genotype` = case_when(
                          str_detect(Isolate, "_") ~ str_extract(Isolate, "^[^_]+"),
                          TRUE ~ Isolate
                        )) %>%
                      dplyr::select(`Parental genotype`,npg_run_id),
...
``` {r Do any samples have this methylase}

# Methylase: group_2030.
non_specific_methylase_carriers <-
  Hinflu_gene_preabsence %>% 
    dplyr::filter(Gene == "group_2030" & CDS != "") %>% 
    dplyr::pull(npg_run_id)

long_read_non_specific_methylase_carriers <-
  long_read_sample_df %>%
    dplyr::filter(npg_run_id %in% non_specific_methylase_carriers)
...
# DNA methylation downstream analysis


## Import sample metadata

```{r}
Run1 <- fread("sample_list_run1.txt", header = FALSE)
Run2 <- fread("sample_list_run2.txt", header = FALSE)

colnames(Run1) <- colnames(Run2) <-
  c("Assembly","Bam","Sample","Species")

Long_read_sample_df <-
  bind_rows(Run1, Run2) %>%
  select(Sample, Species)

head(Long_read_sample_df)
```

This step imports sample metadata and species information for all long-read sequenced isolates.

# Import methylation motif calls
...
for(f in files){
  ...
  df <- fread(paste0("motifs/", f))

  Sample <- substr(f, 1, nchar(f)-11)
...
df_meth <- bind_rows(all)

head(df_meth)
```

Methylation motifs identified by MicrobeMod were combined into a single dataset for downstream analysis.

# Data cleaning and filtering

```{r}
df_meth$Methylated_sites <-
  as.numeric(df_meth$Methylated_sites)

df_meth$Average_Percent_Methylation_per_site <-
  as.numeric(df_meth$Average_Percent_Methylation_per_site)
...
Samples containing only “No Motif Assigned” calls were separated from samples with identified motifs.

```{r}
sample_no_motif <- df_meth %>%
  group_by(Sample) %>%
  summarise(
    n_motifs = n_distinct(Motif),
    only_no_motif =
      all(Motif == "No Motif Assigned")
  ) %>%
  filter(only_no_motif) %>%
  pull(Sample)
```

# Merge methylation and species metadata

```{r}
df1_meth = df_meth%>%filter(!Sample%in%sample_no_motif)
df1_meth = df1_meth%>%filter(Motif!="No Motif Assigned")
df2_meth = df_meth%>%filter(Sample%in%sample_no_motif)

df_meth = bind_rows(df1_meth, df2_meth)
...
# Heatmap visualisation of methylation motifs

## Heatmap function

```{r Motifs heatmap}
plot_species_heatmap <- function(sp){
  ...
  x <- df_meth2 %>%
    ...
    select(
      Sample,
      Motif,
      Average_Percent_Methylation_per_site
    ) %>%
    pivot_wider(
      names_from = Motif,
      values_from =
...
Heatmaps were generated to compare methylation motif distributions across species.

## Moraxella catarrhalis heatmap
...
## Haemophilus influenzae heatmap
...
## Import ML probability histograms

```{r Import Ml probability histograms}
passed <- readLines("ML_Prob/passed.txt")
failed <- readLines("ML_Prob/failed.txt")
No_Motifs <- df_meth2 %>% select(Sample,Motif)%>%
  group_by(Sample) %>%
  filter(n() == 1 & Motif == "No Motif Assigned") %>%
  pull(Sample)
passed = passed[!passed%in%No_Motifs]

# get histogram files
files <- list.files(path ="ML_Prob/" ,pattern="_histogram.txt$")

all <- list()

for(f in files){
  
  df <- read.table(paste("ML_Prob/",f,sep = ""), header=FALSE)
  colnames(df) <- c("prob","count")
  
  Sample <- substr(f,1,nchar(f)-17)
  
  status <- ifelse(Sample %in% passed,"With Assigned Motif",
                   ifelse(Sample %in% failed,"No Methylation Detected",
                    ifelse(Sample %in% No_Motifs,"No Motif Assigned","NA")))
...
dat <- dat %>% dplyr::left_join(Long_read_sample_df,by="Sample")

# weighted mean ML probability per sample
means <- dat %>%
  group_by(Sample,status) %>%
  summarise(mean_prob = weighted.mean(prob,count),
            total=sum(count))%>%
  dplyr::left_join(Long_read_sample_df,by="Sample")
```

Modification likelihood (ML) probability distributions were analysed to compare methylation confidence between samples.

# Density plot of ML probabilities
...
# Mean ML probability per sample

Modification likelihood (ML) probabilities were summarised per sample to compare methylation confidence across isolates with assigned motifs, no assigned motifs, and no detectable methylation.
...
p2 <- ggplot(
  ...
  facet_wrap(~Species) +
  labs(
    title = "Mean ML Probability per Sample",
    x = "Methylation status",
    y = "Mean modification probability"
  ) +
...
# Per-sample ML probability histograms

## Moraxella catarrhalis

Per-sample histograms were generated to visualise ML probability distributions across *Moraxella catarrhalis* isolates.
...
p3 <- ggplot(
  ...
  labs(
    title = "Per-sample ML Histograms",
...
Per-sample histograms were generated to visualise ML probability distributions across *Haemophilus influenzae* isolates.
...
# Restriction–modification system analysis


```{r}
MicroMod_an_Passed = fread("annotate_rm/passed_all_rm_genes.tsv")
MicroMod_an_Passed_MT <- MicroMod_an_Passed %>% filter(Gene_type == "MT")%>%
  mutate(Status="With Assigned Motif")
MicroMod_an_Passed_MT[MicroMod_an_Passed_MT$Sample%in%No_Motifs,"Status"]="No Motif Assigned"

MicroMod_an_Failed = fread("annotate_rm/failed_all_rm_genes.tsv")
MicroMod_an_Failed_MT <- MicroMod_an_Failed %>% filter(Gene_type == "MT")%>%
  mutate(Status="No Methylation Detected")
MicroMod_an_Passed_n <- MicroMod_an_Passed_MT %>%
  dplyr::count(Sample) %>%
  mutate(Group="With Assigned Motif")
MicroMod_an_Passed_n[MicroMod_an_Passed_n$Sample%in%No_Motifs,"Group"]="No Motif Assigned"


MicroMod_an_Failed_n <- MicroMod_an_Failed_MT %>%
  dplyr::count(Sample) %>%
  mutate(Group="No Methylation Detected")

df <- bind_rows(MicroMod_an_Passed_n, MicroMod_an_Failed_n)


df = df %>%
  dplyr::left_join(Long_read_sample_df,
                   by="Sample")
p5=ggplot(df, aes(Group, n, fill=Group)) +
  geom_boxplot() +
  facet_wrap(~Species)+
  geom_jitter(width=0.15) +
  ylab("Number of MT genes")+
  theme(axis.text.x =  element_text(angle = 90, vjust = 0.5, hjust = 0.5),
    panel.grid = element_blank(),
  ...
  labs(title = "Number of MT genes per sample")
p5
All_MT_data = rbind(MicroMod_an_Failed_MT,MicroMod_an_Passed_MT)
...
# ---------------------------------------
# keep only MT genes with homolog names
# ---------------------------------------
x <- All_MT_data[
  Gene_type == "MT" &
    REBASE_homolog != "" &
    !is.na(REBASE_homolog)
]
x <- x %>%
  mutate(
    Homolog_methylation = ifelse(
      Homolog_methylation == "" | is.na(Homolog_methylation),
      "unknown",
      Homolog_methylation
    )
  )
...
colnames_MT = c("Sample_file","Query","Subject","Percent_identity",
                "Alignment_length","Mismatches","Gap_opens","Query_start",
                "Query_end","Subject_start","Subject_end","Evalue","Bit_score")

m4c = fread("annotate_rm/rm_m4C_summary.tsv")
m5c = fread("annotate_rm/rm_m5C_summary.tsv")
m6A = fread("annotate_rm/rm_m6A_summary.tsv")
unk_MT = fread("annotate_rm/rm_unknown_MTase.tsv")
colnames(m4c)=colnames(m5c)=colnames(m6A)=colnames(unk_MT)<-colnames_MT

MT_Genes_alignment_Info = bind_rows(m4c,m5c,m6A,unk_MT)
MT_Genes_alignment_Info = MT_Genes_alignment_Info%>% mutate(
  Sample=substr(Sample_file,1,nchar(Sample_file)-9))%>% 
  select(-Sample_file)%>%select(Sample,everything())%>%
  mutate(
    MT_type = case_when(
      str_detect(Subject, "m6A") ~ "m6A",
      str_detect(Subject, "m4C") ~ "m4C",
      str_detect(Subject, "m5C") ~ "m5C",
      str_detect(Subject, "--")  ~ "unknown",
      TRUE ~ "other"
    )
  )

MT_Genes_alignment_Info <- MT_Genes_alignment_Info %>%
  separate_wider_delim(Subject,delim = ":",names = c("New1","New2"))%>%
  separate_wider_delim(New2,delim = "-",names = c("REBASE_homolog","System_Type","Homolog_methylation","Homolog_motif"),
                       too_many  = "merge")%>%
  mutate(
    System_Type = System_Type %>%
      str_remove("^putative_") %>%           
      str_remove("_methyltransferase$")      
  )%>%
  select(-New1)%>%
  mutate(System_Type = paste("RM_",System_Type,sep = ""))%>%
  select(Sample,Query,REBASE_homolog,System_Type,Homolog_methylation,Homolog_motif,Alignment_length)%>%
  mutate(
    Homolog_methylation = ifelse(
      is.na(Homolog_methylation) | Homolog_methylation == "",
      "unknown",
      Homolog_methylation
    )
  )

colnames(MT_Genes_alignment_Info)[2]="Gene"

# best_hits <- MT_Genes_alignment_Info %>%
#   group_by(Sample, Homolog_methylation) %>%
#   slice_max(order_by = Alignment_length, n = 1, with_ties = FALSE) %>%
#   ungroup()

x <- x%>% 
  
  dplyr::left_join(MT_Genes_alignment_Info,
                   by=c("Sample","Gene","REBASE_homolog",
                      "System_Type","Homolog_methylation","Homolog_motif"))
All_MT_data = rbind(MicroMod_an_Failed_MT,MicroMod_an_Passed_MT)
...
# Methyltransferase heatmaps
## Moraxella catarrhalis methyltransferase heatmap

The distribution of methyltransferase homologs across *Moraxella catarrhalis* isolates was visualised using clustered heatmaps based on alignment lengths.
...
mat <- dcast(
  mat_dt,
  RowID ~ REBASE_homolog,
  value.var = "Alignment_length",
  fun.aggregate = max,
  fill = 0
)
...
pheatmap(
  ...
  fontsize_col = 6,
  border_color = NA,
  main = paste("MT genes:", sp),

  color = viridis::viridis(100),

  annotation_colors = list(
    Status = c(
      `With Assigned Motif` = "green",
      `No Motif Assigned` = "orange",
      `No Methylation Detected` = "red"
    )
  )
)
```

## Haemophilus influenzae methyltransferase heatmap

The distribution of methyltransferase homologs across *Haemophilus influenzae* isolates was visualised using clustered heatmaps based on alignment lengths.
...
Restriction–modification system annotations generated by MicrobeMod were used to characterise methyltransferase families across isolates.

# Motif summary table

## Create motif families
...
df2 <- df_meth2 %>%
  mutate(
    Motif_family = case_when(
      ...
      str_detect(Motif, "GCGC") ~ "GCGC-like",

      str_detect(Motif, "GCGCNT") ~ "GCGC-like",

      str_detect(Motif, "GCGCNY") ~ "GCGC-like",

      str_detect(Motif, "GCGCY") ~ "GCGC-like",

      str_detect(Motif, "GCGCYT") ~ "GCGC-like",

      str_detect(Motif, "RGCGCY") ~ "GCGC-like",

      str_detect(Motif, "No Motif Assigned") ~ "No motif assigned",
...
Motifs were grouped into broader motif families to simplify interpretation and facilitate comparison between *Moraxella catarrhalis* and *Haemophilus influenzae* methylation profiles.

# Summarise motifs per species
...
Summary statistics were calculated for each motif, including prevalence, methylation levels, and coverage across samples.

# Collapse singleton motifs
...
summary_table2 <- summary_table %>%
  ...
  summarise(
    ...
    Mean_genome_sites =
      round(
        mean(Mean_genome_sites),
        1
      ),

    Mean_methylated_sites =
      round(
        mean(Mean_methylated_sites),
        1
      ),
...
Rare motifs identified in single isolates were collapsed into a single “Singleton motifs” category to improve table readability.

# Generate publication-ready table

```{r}
final_gt <- summary_table2 %>%
  ...
  tab_header(
    title = "Summary of methylated motifs by species"
  ) %>%
  ...
  cols_label(
    ...
    Mean_genome_sites = "Mean genome sites",
    Mean_methylated_sites = "Mean methylated sites",
    Prevalence = "Prevalence"
  ) %>%

  data_color(
    columns = Mean_methylation,
...
The final table summarises methylation motif distributions and prevalence across both bacterial species.


# Restriction–modification methyltransferase family distribution

## Summarise methyltransferase family frequencies

```{r}
Type_RM <- as.data.frame(
  table(
    All_MT_data$HMM,
    All_MT_data$Species
  )
...
Restriction–modification methyltransferase families were quantified across *Moraxella catarrhalis* and *Haemophilus influenzae* isolates to compare species-specific R–M system composition.


# Bubble plot of R–M families
...
The bubble plot highlights species-specific differences in the abundance and distribution of restriction–modification methyltransferase families between *Moraxella catarrhalis* and *Haemophilus influenzae*.
...
previous_treatment_df <-
  infant_swabs_df %>%
    dplyr::left_join(treatment_df, by = "codenum", relationship = "many-to-many") %>%
...
treatment_sampling_df <-
  infant_swabs_df %>%
    dplyr::left_join(previous_treatment_df, by = "specnum") %>%
    dplyr::left_join(next_treatment_df, by = "specnum") %>% 
    dplyr::right_join(
      sample_df %>% 
...
To answer this question, I used the Kraken/Bracken output to perform the following steps:    
1. Generated an abundance table at the species level.  
2. Filtered out all single-infection samples.  
3. Performed a Pearson correlation test across species to obtain a correlation matrix.  
4. Visualised the results using ggplot2.  


``` {r Pearson correlation analysis for competition or association between species analysis }
Mixed_bacteria_samples <- parse_bracken("./sample_data/6588_bracken_summary_report.tsv.gz")%>%
  mutate(Species_Detected1 = str_extract(Classification, "[^|]+$")) %>%
  mutate(Species_Detected2 = str_extract(Species_Detected1, "[^_]+_[^_]+$")) %>%
  mutate(Species=str_replace(Species_Detected2,"_"," "))%>%
  select(-Classification,-Species_Detected1,-Species_Detected2)%>%
  select(Species,everything())%>%
filter(
  str_count(Species, "\\S+") == 2,            # keep only two-word names
  !str_detect(Species, "\\[.*\\]"),           # remove brackets
  !str_detect(Species, "\\(.*\\)"),
  !str_detect(Species, "^\\d") ,             # remove strings starting with a number
  str_detect(Species, "^[A-Z][a-z]+\\s[a-z]+$"),
  !str_detect(Species, "ATCC|FSL|DSM|taxon|sp|PCC"), # remove 'sp' 'PCC' 'ATCC' etc
  Species%in%species_sample_df$Detected_species,
  Species!="Homo sapiens")%>% # keep species in species_sample_df 
column_to_rownames("Species") %>%
t() %>%
as.data.frame() %>%
rownames_to_column(var ="npg_run_id" )%>%
 dplyr::left_join(
  species_sample_df %>% select(npg_run_id, Country, Description),
  by = "npg_run_id"
 )%>%
  distinct(npg_run_id, .keep_all = TRUE)%>%
mutate(npg_run_id = as.factor(npg_run_id))%>%
 column_to_rownames(var = "npg_run_id")%>%
  filter(Description=="Mixed bacteria")

cor_matrix_Thailand <- cor(Mixed_bacteria_samples%>%filter(Country=="Thailand")%>%select(-c(Country,Description)), method = "spearman")
cor_matrix_Cambodia <- cor(Mixed_bacteria_samples%>%filter(Country=="Cambodia")%>%select(-c(Country,Description)), method = "spearman")

melted_Thailand <-reshape2:: melt(cor_matrix_Thailand)
melted_Thailand$Country = "Thailand"
melted_Combodia <- reshape2::melt(cor_matrix_Cambodia)
melted_Combodia$Country = "Cambodia"

species_co_colonisation_per_country = bind_rows(melted_Thailand,melted_Combodia) 
species_co_colonisation_per_country = na.omit(species_co_colonisation_per_country)
species_co_colonisation_per_country$Country = factor(species_co_colonisation_per_country$Country)
```
#### Visualisation of pearson correlation: Species co_colonisation in mixed infection from deep sequencing data in Cambodia and Thailand 

``` {r Visualisation of pearson correlation }
p1=ggplot(data = species_co_colonisation_per_country, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile()+
  ...
  theme(
    ...
    legend.text = element_text(size = 16),
       axis.text.x = element_text(angle = 90,size = 14, vjust=.5, hjust=1),
        axis.text.y = element_text(size = 13, vjust=.5, hjust=1))+
  labs(x="", y="")
p1
```


#### Visualisation of pearson correlation:Species co_colonisation in mixed infection per Country 

``` {r Visualisation of pearson correlation accross Thailand and Cambodia }
p2=ggplot(data = species_co_colonisation_per_country, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile()+
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
                       midpoint = 0, limit = c(-1,1), space = "Lab", 
                       name="Pearson\nCorrelation")+
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 90,size = 12, vjust=.5, hjust=1),
        axis.text.y = element_text(size = 10, face = "bold", vjust=.5, hjust=1))+
  labs(x="", y="")
p2
```


This analysis revealed a **competition** between *Haemophilus influenzae*, and *Moraxella catarrhalis*.  

To confirm this observation, we clustered samples/species using bluster/TreeSummarizedExperiment/mia packages.  

To do this:   

1. I first created a tse object using TreeSummarizedExperiment.  
2. I Filtered out species with low proportion to ease the calculation.  
3. I added clusters with addcluster and HclustParam functions.  
4. I generated a table with the number of samples contributing for each cluster.  
5. I got the hclust data from metadata.  
6. I used as.dendrogram and color_branches functions from dendextend package to convert hclust data and color the graph.  

#### Add sample cluster
``` {r Samples clustering}                  
metadata = species_sample_df%>%
filter(npg_run_id%in%rownames(Mixed_bacteria_samples))%>%
  distinct(npg_run_id, .keep_all = TRUE) %>%
  column_to_rownames(var = "npg_run_id")

tse <- TreeSummarizedExperiment(
  assays = list(counts = Mixed_bacteria_samples%>%
...
altExp(tse, "prevalent") <- agglomerateByPrevalence(
  tse,
  rank = NULL, prevalence = 20 / 100, detection = 1 / 100
)
...
cols <- cols[c( 2, 1, 3,4,5,6)]
dend <- color_branches(dendro, k = k, col = unlist(cols))

# Label the samples by their dominant taxon

alt <- altExp(tse, "prevalent")

assay(alt) <- as.matrix(assay(alt))

altExp(tse, "prevalent") <- addDominant(alt)
...
#### Dominant species per clester.  

Dominant species were extracted from individual samples across clusters to identify species contributions to each cluster.  

A summary table was generated to quantify the frequency with which each species appeared as the dominant species within clusters, and corresponding proportions were calculated.  

These proportions were visualized using a bar plot produced with ggplot2.  


```{r species contibution to clusters }
Dominant_taxa_sample = as.data.frame( altExp(tse, "prevalent")$dominant_taxa) %>%
  bind_cols(., as.data.frame(altExp(tse, "prevalent")$hclust))%>%
  rownames_to_column(var="npg_run_id")
colnames(Dominant_taxa_sample) = c("npg_run_id","Dominant_Species","Cluster")

table_Dominant_taxa = as.data.frame(table(Dominant_taxa_sample$Dominant_Species,Dominant_taxa_sample$Cluster))
colnames(table_Dominant_taxa) = c("Dominant_Species","Cluster","Frequency")
list_freq=list()
a=1
for (i in unique(table_Dominant_taxa$Cluster)) {
  df1= table_Dominant_taxa[table_Dominant_taxa$Cluster==i,]
  df1=df1%>%mutate(Proportion=Frequency/sum(Frequency))
  list_freq[[a]]=df1
  a=a+1
}
table_Dominant_taxa = list_freq[[1]]
for (i in 2:length(list_freq)) {
  table_Dominant_taxa = bind_rows(table_Dominant_taxa,list_freq[[i]])
}

species_colors=cols <- c(
  
  # Acinetobacter (Blues)
  "Acinetobacter baumannii" = hcl.colors(9, "Blues")[1],
  "Acinetobacter variabilis" = hcl.colors(9, "Blues")[2],
  
  # Bacillus (Oranges)
  "Bacillus altitudinis" = hcl.colors(9, "Oranges")[1],
  "Bacillus anthracis"   = hcl.colors(9, "Oranges")[2],
  "Bacillus cereus"      = hcl.colors(9, "Oranges")[3],
  "Bacillus safensis"    = hcl.colors(9, "Oranges")[4],
  "Bacillus velezensis"  = hcl.colors(9, "Oranges")[5],
  
  # Enterobacter (Greens)
  "Enterobacter cloacae"    = hcl.colors(9, "Greens")[1],
  "Enterobacter hormaechei" = hcl.colors(9, "Greens")[2],
  
  # Escherichia
  "Escherichia coli" = hcl.colors(1, "Purples"),
  
  # Haemophilus
  "Haemophilus influenzae" = "red", #hcl.colors(1, "YlOrBr"),
  
  # Klebsiella
  "Klebsiella pneumoniae" = hcl.colors(1, "Rocket"),
  
  # Mammaliicoccus
  "Mammaliicoccus sciuri" = hcl.colors(1, "Inferno"),
  
  # Moraxella (YlGnBu)
  "Moraxella bovis"          = hcl.colors(9, "Cyan")[2],
  "Moraxella catarrhalis"    = hcl.colors(9, "Cyan")[1],
  "Moraxella lincolnii"      = hcl.colors(9, "Cyan")[3],
  "Moraxella nonliquefaciens"= hcl.colors(9, "Cyan")[4],
  
  # Neisseria
  "Neisseria subflava" = hcl.colors(1, "Pink"),
  
  # Other
  "Other" = hcl.colors(1, "Grays"),
  
  # Staphylococcus (Magenta)
  "Staphylococcus arlettae"       = hcl.colors(9, "Magenta")[2],
  "Staphylococcus aureus"         = hcl.colors(9, "Magenta")[1],
  "Staphylococcus capitis"        = hcl.colors(9, "Magenta")[3],
  "Staphylococcus epidermidis"    = hcl.colors(9, "Magenta")[4],
  "Staphylococcus haemolyticus"   = hcl.colors(9, "Magenta")[5],
  "Staphylococcus hominis"        = hcl.colors(9, "Magenta")[6],
  "Staphylococcus saprophyticus"  = hcl.colors(9, "Magenta")[7],
  
  # Streptococcus (Blue-Yellow)
  "Streptococcus dysgalactiae" = hcl.colors(9, "Teal")[],
  "Streptococcus mitis"        = hcl.colors(9, "Teal")[2],
  "Streptococcus pneumoniae"   = hcl.colors(9, "Teal")[1],
  "Streptococcus pyogenes"     = hcl.colors(9, "Teal")[4]
  
)

p3=ggplot(table_Dominant_taxa,aes(fill = Dominant_Species,y=Proportion,x=Cluster))+
  geom_bar(stat ="identity" ,position = "stack")+
  scale_fill_manual(values = species_colors)+
  ggtitle("Dominant Species proportion accross Clusters")+
  theme(plot.title = element_text(size = 20,face ="bold.italic", hjust = 0.5, 
                                  margin = margin(t = 10, b = 20, unit = "pt")),
        legend.text = element_text(size = 10),
        strip.text = element_text(face="bold.italic", size=40),
        axis.title.x = element_text(face="bold",size=30),
        axis.title.y = element_text(face="bold",size=30),
        axis.text.x = element_text(angle = 90,size = 20, vjust=.5, hjust=1),
...
### Species co-colonising samples accross clusters.  

Additionally, the proportion of each species within individual samples across clusters was calculated and visualized using a stacked bar plot.    

#### Proportion of species in samples.  


```{r Proportion of species in individual samples }
Mixed_bacteria_samples_cluster<- Mixed_bacteria_samples%>%
  rownames_to_column(var="npg_run_id")%>%
  dplyr::left_join(
    Dominant_taxa_sample %>% select(npg_run_id,Cluster),
    by = "npg_run_id") %>%
  distinct(npg_run_id, .keep_all = TRUE)

Matrix_cluster = Mixed_bacteria_samples_cluster%>%
  select(-Country,-Description,-Cluster)%>%
  column_to_rownames(var="npg_run_id")

Metadata_cluster = Mixed_bacteria_samples_cluster%>%
  select(npg_run_id,Country,Description,Cluster)
  
prop_cluster <- sweep(Matrix_cluster, 1, rowSums(Matrix_cluster), FUN = "/")%>%
 as.data.frame() %>%
  rownames_to_column(var="npg_run_id")%>%
  pivot_longer(
    cols = -npg_run_id,
  ...
  dplyr::left_join(Metadata_cluster, by = "npg_run_id")%>%
  arrange(Cluster, npg_run_id) %>%
  mutate(npg_run_id = factor(npg_run_id, levels = unique(npg_run_id)))

p4=ggplot(prop_cluster, aes(x = Proportion, y = npg_run_id, fill = Species)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ Cluster, scales = "free_y", ncol = 1,strip.position = "right") +
  scale_fill_manual(values = species_colors)+
  labs(x = "Proportion", y = "Samples") +
  theme_bw()+
  theme(strip.text.y.left = element_text(angle = 0),
        axis.text.x = element_text(size = 20, vjust=.5, hjust=1),
        axis.title.x = element_text(face="bold", size=30),
        axis.title.y = element_text(face="bold", size=30),
...
#### Proportion of top 10 species in samples.    

```{r Pproportion of top 10 species in individual samples }
top_species <- prop_cluster %>%
  group_by(Species) %>%
  summarise(total = sum(Proportion)) %>%
  top_n(10, total) %>%
  pull(Species)

  prop_cluster$Species <- ifelse(prop_cluster$Species %in% top_species,
                                 prop_cluster$Species, "Other")
  prop_cluster <- prop_cluster %>%
    group_by(Cluster, npg_run_id) %>%
    mutate(total = sum(Proportion)) %>%
    ungroup() %>%
    arrange(Cluster, total) %>%
    mutate(npg_run_id = factor(npg_run_id, levels = unique(npg_run_id)))
  
p5=ggplot(prop_cluster, aes(x = Proportion, y = npg_run_id, fill = Species)) +
...
After sample clustering, I clustered species using bluster/TreeSummarizedExperiment/mia packages.  

#### Add species cluster 
To add species cluster:   
1. I first calculated the distance between species.  
2. I converted the distance matrix into hclust object and use cutree function to add specie clusters.  
3. I ordered clusters and ploted using r base.  

```{r Species clustering }
mat <- assay(tse)
dist_mat <- dist(mat, method = "euclidean") # distance between species
hc <- hclust(dist_mat)

clusters <- cutree(hc, k = 6)  # 
ord <- hc$order
clusters_ordered <- clusters[ord]
cluster_order <- unique(clusters_ordered)
cols <- 2:(k+1)

plot(hc, labels = FALSE, main = "Species Clustering Dendrogram",
     xlab = "Species", sub = "")

rect.hclust(hc, k = k, border = cols)

# Species names
species <- rownames(tse)

# Cluster list
cluster_list <- split(species, clusters)

# Reorder cluster list to match rectangles
cluster_list <- cluster_list[as.character(cluster_order)]

legend_labels <- sapply(seq_along(cluster_list), function(i) {
  if (i == 6) {
    paste("Cluster", i, ":", paste(cluster_list[[i]][1])," and other species" ) # only first species
  } else {
    paste("Cluster", i, ":", paste(cluster_list[[i]], collapse = ", "))
  }
})
legend("topright",
...
#### UMAP Analysis of Mixed Bacterial Samples to Reveal structure.   

To further explore patterns in species composition, a UMAP analysis was performed on species abundance data using the UMAP package, and samples were colored according to previously defined clusters.  

```{r UMAP analysis }
umap_samples = umap(Mixed_bacteria_samples%>%select(-Country,-Description),
                    n_components = 2, random_state = 15)
layout <- umap_samples[["layout"]] 
layout <- data.frame(layout) 
umap_values_per_country <- cbind(layout, Mixed_bacteria_samples$Country) 
colnames(umap_values_per_country)[3] = "Country"
umap_values_per_country<- umap_values_per_country%>%
  rownames_to_column(var="npg_run_id")%>%
  dplyr::left_join(
    Dominant_taxa_sample %>% select(npg_run_id,Cluster),
    by = "npg_run_id"
...
p6=ggplot(umap_values_per_country, aes(x = X1, y = X2,colour = Cluster)) +
  geom_point(size=4,aes(shape = Country)) +
  theme(legend.title = element_text(size=20), 
        plot.title = element_text(size=40),
        legend.text = element_text(size = 20),
        strip.text = element_text(face="bold", size=40),
        axis.text.x = element_text(size = 36, vjust=.5, hjust=1),
        axis.title.x = element_text(face="bold", size=40),
        axis.title.y = element_text(face="bold", size=40),
        axis.text.y = element_text(size = 36, face = "bold", vjust=.5, hjust=1)
  )+
  #scale_color_manual(values =c("#F8766D", "#7CAE00", "#00BFC4", "#C77CFF") )+
  labs(
    x="UMAP1",
    y="UMAP2",
    title = "UMAP of Mixed Infection Samples Across Two Countries")
p6
...
Metagenomic samples enriched for *Moraxella catarrhalis* were assembled using SPAdes prior to downstream comparative genomics and GWAS analysis.
...
# Kraken2 filtering of *Moraxella catarrhalis* contigs

To minimise contamination from mixed metagenomic assemblies, contigs assigned to *Moraxella catarrhalis* were extracted using Kraken2 classification and SeqKit filtering.
...
SPADES_DIR="spades_mcat_only"
OUTDIR="spades_mcat_kraken_filtered"

THREADS=8
TAXID=480

for f in "$SPADES_DIR"/*.fasta
do
    ...
    awk -v tax="$TAXID" \
      '$3==tax {print $2}' \
      "$OUTDIR/${sample}.kraken.out" \
      > "$OUTDIR/${sample}.mcat.ids"

    seqkit grep \
      -f "$OUTDIR/${sample}.mcat.ids" \
      "$f" \
      > "$OUTDIR/${sample}.Mcat.fasta"

done
```

Only contigs classified as *Moraxella catarrhalis* were retained for downstream unitig-based GWAS analysis.

# Unitig calling

Unitigs were generated from filtered *Moraxella catarrhalis* assemblies using `unitig-caller`.

```bash
#!/bin/bash

module load unitig-caller/1.3.0

unitig-caller \
    --call \
    --refs Mcat_clean_list.txt \
    --pyseer \
    --out unitig_mcat \
    --kmer 31 \
    --threads 8
...
Unitig presence–absence matrices were generated for Pyseer-compatible bacterial GWAS analysis.

# Mash distance matrix generation

Pairwise Mash distances were calculated to estimate genome similarity between *Moraxella catarrhalis* isolates.

```bash
#!/bin/bash

module load mash/2.1.1

find spades_mcat_kraken_filtered \
  -name "*.Mcat.fasta" \
  > assemblies.txt
...
mash dist \
    assemblies.txt.msh \
    assemblies.txt.msh \
    > mcat_mash_square.tsv
```

# Convert Mash distances into kinship matrix

```{r}

mash <- fread(
  "Mcat_results/mcat_mash_distances.clean.tsv",
  header = FALSE
)
...
write.table(
  kinship,
  "Mcat_results/mcat_mash_kinship_matrix.tsv",
  sep = "\t",
  quote = FALSE
)
```

Mash distances were converted into a kinship matrix to account for bacterial population structure during GWAS analysis.

# Pyseer GWAS analysis

A linear mixed-model GWAS was performed using Pyseer with the Mash kinship matrix and unitig presence–absence data.

```bash
#!/bin/bash

module load pyseer/1.4.0

pyseer \
  --phenotypes Mcat_phenotype_pyseer.txt \
  --kmers unitig_mcat_fixed.pyseer.gz \
  --similarity mcat_mash_kinship_matrix.tsv \
  --lmm \
  --min-af 0.01 \
  --cpu 8 \
  > mcat_pyseer_unitig_results_update.txt
```

Significant unitigs associated with *M. catarrhalis* co-colonisation phenotypes were identified using a linear mixed-model framework controlling for genomic relatedness.

# Extract significant unitigs
...
Significant unitigs were subsequently annotated and visualised using downstream R analyses and Manhattan plots.

## Downstream analysis


```{r gwas_downstream_analysis, message=FALSE, warning=FALSE}

## -------------------------------------------
## Load pyseer annotated significant hits
## -------------------------------------------

df <- fread(
  "Mcat_results/all_annotated_hits.clean.tsv",
  header = FALSE
)
...
df[, logp := -log10(as.numeric(lrt_p))]

## cap extremely significant values
df[, logp := pmin(logp, 50)]
...
df2 <- df %>%
  mutate(
    highlight = case_when(
      ...
      str_detect(gene, "uspA1") &
        pos >= 1194300 &
        pos <= 1197167 ~ "uspA1",

      str_detect(gene, "CRISPR") ~ "CRISPR",
...
## -------------------------------------------
## Restrict to reference chromosome
## -------------------------------------------
...
ggplot(main, aes(x = pos, y = logp)) +
  ...
  labs(
    title = "Manhattan plot (CP002005.1)",
    x = "Genome position",
    y = "-log10(p-value)"
  )

## -------------------------------------------
## Highlight significant k-mers
## -------------------------------------------

sig <- main[logp > 10]

p1=ggplot(main, aes(pos, logp)) +
  ...
  labs(
    x = "Genomic position (CP002005.1)",
    y = expression(
      -log[10](italic(p)) ~ "(LRT test)"
    )
  )

## -------------------------------------------
## Define important genomic regions
## -------------------------------------------

genes <- data.table(
  gene  = c("uspA2H", "mid", "uspA1H"),
  chrom = "CP002005.1",
  start = c(346375, 627743, 1196528),
  end   = c(346406, 627774, 1196583)
)
...
## -------------------------------------------
## Final annotated Manhattan plot
## -------------------------------------------
...
p2 <- ggplot(
  ...
  geom_point(
    data = subset(main, highlight != "other"),
    aes(color = highlight),
    size = 2
  ) +
