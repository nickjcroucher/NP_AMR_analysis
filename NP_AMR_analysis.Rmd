...
### Compare resistance between *H. influenzae* and *M. catarrhalis*
...
compare_strain_frequencies <- function(df) {
  
  freq_wide <- df %>%
      count(Cohort, Cluster, name = "n") %>%
      group_by(Cohort) %>%
      mutate(prop = n / sum(n)) %>%
      ungroup() %>%
      tidyr::pivot_wider(
...
compare_strain_frequencies(Hflu_microreact_df)
...
``` {bash Assembling long-read data}
...
## Co-colonisation analysis across countries

## Resistance relative to drug administration
